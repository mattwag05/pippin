...
Top-level `resolveContacts: Bool` in `config.json` (sibling of `ai`/`messages`) is the **global default** for resolving Mail/Messages sender/participant handles to Apple Contacts names. Set it to `false` to skip the per-command `CNContactStore` enumeration everywhere — useful for fan-out callers (the morning briefing runs `mail list` per account + `messages list`, so resolution-on means N+1 address-book enumerations per run). Absent/`nil` keeps resolution **ON** (non-breaking). Precedence is `--no-contacts`/`--contacts` flag > `resolveContacts` config > default-ON, computed by `AIProviderFactory.shouldResolveContacts` (`pippin/AIProvider/AIProviderFactory.swift`); `--contacts` was added so the flag can force-enable over a disabling config. Phone matching is digits-only with a last-10-digit fallback (drops a leading country code) — see README § Contact Name Resolution for the cross-country-code collision caveat (first-write-wins, shown as a confident name with no fuzzy-match signal).
