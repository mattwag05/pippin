...
| [local-ci.md](local-ci.md) | Running CI locally in an ephemeral macOS VM (`make ci-vm`) now that the GitHub `ci.yml` workflow is gone. |
...
| [agent-prompts/](agent-prompts/) | Live prompts for `/autonomous-execution` runs (audit, review-and-ship). |
| [archive/](archive/) | Dated historical implementation plans. Read-only — shipped features only. |
| [superpowers/specs/](superpowers/specs/) | Approved design specs kept for architectural context. |
