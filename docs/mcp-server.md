...
- **Prefer the MCP tools where they're attached** — the Hermes/Talia gateway and Claude Cowork (via the `pippin@mw-plugins` plugin) both register `pippin mcp-server`, so `mail_list`, `calendar_today`, etc. are first-class tools there.
...
- **Talia (Hermes-Agent on M5)** — registers `pippin mcp-server` as a stdio MCP and drives the 45 tools natively (see the operator's Hermes setup notes).
